...
bool CNullDriver::checkPrimitiveCount(u32 prmCount) const
{
	...
	{
		char tmp[1024];
		snprintf(tmp, sizeof(tmp), "Could not draw triangles, too many primitives(%u), maximum is %u.", prmCount, m);
...
CNullDriver::SHWBufferLink *CNullDriver::getBufferLink(const scene::IMeshBuffer* mb)
{
	...
	SHWBufferLink *Link = static_cast<SHWBufferLink*>(mb->getHWBuffer());
	if (Link)
		return Link->Owner == this ? Link : 0;

	Link = createHardwareBuffer(mb);
	if (Link)
	{
		Link->Owner = this;
		mb->setHWBuffer(Link);
...
{
	SHWBufferLink *Link = mb ? static_cast<SHWBufferLink*>(mb->getHWBuffer()) : 0;
